// Train mode. bryan
//
// The default way to play: mines puzzle positions from the user's imported
// games and drills them. The mining knobs (which position tags qualify) are
// set by the app — the user no longer picks easy/medium/hard; they just pick
// Train vs Rush at import time and each mode owns its own tuning.
//
// When a new position loads the mode rewinds the board to the FEN *before*
// the opponent's last move and then animates that move forward, so the user
// sees the move that landed them here before they're allowed to respond.
//
// On submit:
...
// Proprietary mining tuning for train mode. Not user-configurable — tune here
// as the mode's difficulty curve is refined.
const TRAIN_POSITION_FILTERS: PuzzleTags[] = ["easy", "medium"];

function useTrainMode(ctx: ModeContext): ModeReturn {
  const latestRequestRef = useRef(0);
  const latestMiningRunRef = useRef(0);
  const minedGamesRef = useRef<SessionData["games"] | null>(null);
  ...
  const minePuzzles = useCallback(
    async (runId: number) => {
      ...
      try {
        await minePuzzlePositions(
          ...
          {
            filters: [...TRAIN_POSITION_FILTERS],
    ...
    [ctx.appendTrainingPosition, ctx.sessionData?.games, ctx.updateSessionData],
  );

  useEffect(() => {
    const games = ctx.sessionData?.games;
    if (!games || games.length === 0 || minedGamesRef.current === games) {
      return;
    }

    minedGamesRef.current = games;
  ...
  }, [ctx.sessionData?.games, minePuzzles]);
  ...
  const onMoveSubmit = useCallback(
    async (uci: string, fenAfterMove: string) => {
      ...
      try {
        ...
        window.clearTimeout(spinnerTimer);
        console.error("Train mode engine call failed:", error);
        if (isStillRelevant()) {
          ctx.boardRef.current?.clearLoadingBadge();
        }
      }
